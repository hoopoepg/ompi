...
static void mca_spml_ikrit_cache_mkeys(shmem_ctx_t ctx, sshmem_mkey_t *, uint32_t seg,
                                       int remote_pe, int tr_id);
                                         ...
                                         int *count)
{
    ...
    *count = 0;
    ...
    for (i = 0; i < MXM_PTL_LAST; i++) {
        ...
        mca_spml_ikrit_cache_mkeys(oshmem_ctx_default, &mkeys[i], memheap_find_segnum(addr), my_rank, i);
    }
    *count = MXM_PTL_LAST;
...
int mca_spml_ikrit_oob_get_mkeys(shmem_ctx_t ctx, int pe, uint32_t seg, sshmem_mkey_t *mkeys)
{
    ...
    if (mca_spml_ikrit.ud_only) {
        ...
        mca_spml_ikrit_cache_mkeys(oshmem_ctx_default, &mkeys[ptl], seg, pe, ptl);
        return OSHMEM_SUCCESS;
    }
